  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#0a0a0a] font-mono transition-colors duration-300" style={{ paddingBottom: '120px' }}>
      <div className="container-main" style={{ paddingTop: '120px' }}>
        ...
        {/* Back nav */}
        <div className="mb-12">
          <Link to="/" className="text-[0.7rem] text-gray-300 dark:text-gray-600 hover:text-accent transition-colors tracking-[0.3em] uppercase font-black flex items-center gap-4 group">
        ...
        <div className="mb-12">
          ...
          <div className="flex flex-wrap gap-4 text-[0.7rem] font-black tracking-[0.15em]">
            <button
              ...
              className={`transition-colors ${filter === 'ALL' ? 'text-accent' : 'text-gray-300 dark:text-gray-600 hover:text-black dark:hover:text-white'}`}
            >
            ...
            {ALL_VENUES_EN.map(v => (
              <button
                key={v}
                onClick={() => setFilter(v)}
                className={`transition-colors ${filter === v ? 'text-accent' : 'text-gray-300 dark:text-gray-600 hover:text-black dark:hover:text-white'}`}
              >
                [{v.toLowerCase().replace(/ /g, '-')}]
              </button>
            ))}
            <span className="ml-auto text-[0.6rem] text-gray-200 dark:text-gray-700 self-center hidden sm:block">
        ...
        <div className="hidden md:grid grid-cols-12 gap-4 text-[0.6rem] text-gray-200 dark:text-gray-700 font-black tracking-[0.35em] uppercase pb-4 border-b border-gray-100 dark:border-[#222]">
              ...
              >
                ...
                >
                  ...
                  <div className="flex items-center justify-between w-full md:hidden">
                    <span className="text-[0.6rem] text-gray-300 dark:text-gray-600">#{String(i + 1).padStart(2, '0')}</span>
                    <span className="text-[0.6rem] text-accent font-black">{pub.code} →</span>
                  </div>

                  {/* REF — desktop only */}
                  <span className="hidden md:block col-span-1 text-[0.65rem] text-gray-200 dark:text-gray-700 group-hover:text-accent transition-colors pt-1">
                  ...
                  {/* Venue */}
                  <span className="text-[0.65rem] text-gray-400 dark:text-gray-500 tracking-[0.1em] uppercase md:col-span-3">
                    {getVenue(pub) || '—'}
                  </span>

                  {/* Year */}
                  <span className="text-[0.65rem] text-gray-300 dark:text-gray-600 md:col-span-1">
        ...
        {/* Terminal footer */}
        <div className="mt-12 flex items-center gap-3 text-[0.75rem] text-gray-300 dark:text-gray-600">
